...
test('Cloudflare runtime: private enrollment, exact target delivery, pause, replay and revocation', async t => {
  ...
  const mf = new Miniflare(convertV4MiniflareOptions({ workers: [{ name: 'claw-test', modules: [
    ...
    ...['bootstrap.js', 'tenancy.js', 'onboarding.js', 'catalog.js', 'status.js', 'accounts.js', 'panel.js', 'panel-controller.js', 'batch.js', 'features.js', 'actions.js', 'alerts.js', 'feature-commands.js'].map(name => ({ type: 'ESModule', path: fileURLToPath(new URL('../' + name, import.meta.url)) })) ],
...
test('deployment pairing works through real session/socket auth, not an admin HTTP route', async t => {
  ...
  const mf = new Miniflare(convertV4MiniflareOptions({ workers: [{ name: 'seed-test',
    modules: ['worker.js', 'protocol.js', 'bootstrap.js', 'tenancy.js', 'onboarding.js', 'catalog.js', 'status.js', 'accounts.js', 'panel.js', 'panel-controller.js', 'batch.js', 'features.js', 'actions.js', 'alerts.js', 'feature-commands.js'].map(name => ({ type: 'ESModule',
