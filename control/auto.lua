...
local Auto = { VERSION = "0.1.0", LOBBY = 4111023553 }
Auto.__index = Auto
...
function Auto:selectSlot()
	...
	if not self:canAct() or self.profile.slot ~= slot then return self:hold("destination or slot changed before selection") end
...
function Auto:tick()
	...
	if current.placeId == target.ticket.placeId and current.jobId == target.ticket.jobId then
		if profile.slot and current.slot and profile.slot ~= current.slot then return self:statusAs("ATTENTION: wrong character slot") end
	...
	if type(profile.slot) ~= "string" or profile.slot == "" then return self:statusAs("NEEDS_SLOT: load once inside your chosen character") end
	if self.attempt and self.attempt.key == target.key then
		local a = self.attempt
		if a.phase == "DONE" or a.phase == "HOLD" then return self:statusAs("ATTENTION: use /claw retry") end
	...
	self.attempt = { key = target.key, phase = "RETURN_MENU", deadline = self.adapter.now() + 60, startedAt = self.adapter.now() }
	if current.placeId == Auto.LOBBY then return self:selectSlot() end
	if self.adapter.allowMenuReturn ~= true then return self:hold("return to menu disabled in local config") end
	if not self:save() then return end
