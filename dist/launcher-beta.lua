-- Generated by tools/build-control.mjs; private pairing files are read locally, never embedded.
...
do
    ...
    if useBatch then
        if not assert(loadstring([=[-- A temporary shared setup window. Permanent pairing keys remain per account.
...
return false
]=], "@CLAW/batch.lua"))() then return end
    end
end
if getgenv().CLAW_PAIR ~= nil then
assert(loadstring([=[-- One-time setup on one account. Never ships a shared key or a Discord bot token.
local env, Http = getgenv(), game:GetService("HttpService")
...
return assert(loadstring([==[-- Generated by tools/build-control.mjs; edit source modules, not this file.
local __CLAW_BUNDLE = {
