...
Every launch starts inert: Burster, every burst rule, animation logging, Ghost Fire, auto-defense, attack/combat assistance, probability, optional filters/fallbacks, diagnostics, and debug blocks are reset to OFF even if an older settings file saved them as enabled. Tuning values, target lists, key bindings, and explicitly selected action-roll targets remain saved. In v0.4.3, inert also means disconnected: Combat no longer installs its heartbeat, global input listener, high-priority context action, character-state monitor, or detectors until the user explicitly enables Combat or one of its mastered features. Enabling Auto Defense or an assistance feature starts its required Combat runtime automatically. The native bridge retains v0.4.2's state-driven Unblock contract and v0.4.1's paced-churn containment and provenance validation.
