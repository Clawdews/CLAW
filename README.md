...
Every launch starts inert: Burster, every burst rule, animation logging, Ghost Fire, auto-defense, attack/combat assistance, probability, optional filters/fallbacks, diagnostics, and debug blocks are reset to OFF even if an older settings file saved them as enabled. Tuning values, target lists, key bindings, and explicitly selected action-roll targets remain saved. Inert also means disconnected: Combat installs no heartbeat, global input listener, context action, character monitor, or detectors until explicitly enabled. v0.4.4 additionally keeps the main window background input-transparent, never overrides Roblox mouse behavior, and refuses to retain or mutate the game's private input-state upvalue. Native Block/Unblock remains available without that unsafe state; direct dodge/cancel uses configured fallbacks when needed.
