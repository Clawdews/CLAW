...
Every launch starts inert: Burster, every burst rule, animation logging, Ghost Fire, auto-defense, attack/combat assistance, probability, optional filters/fallbacks, diagnostics, and debug blocks are reset to OFF even if an older settings file saved them as enabled. Tuning values, target lists, key bindings, and explicitly selected action-roll targets remain saved. Enabling Auto Defense or an assistance feature automatically enables its required Combat master. The current v0.4.1 build extends Threat Guard with per-attacker and same-animation rearm leases plus longer-lived admission and early-abort churn detection, targeting the paced passive-breaker hole that stayed below v0.4.0's short burst threshold. It also ports Lycoris's player-versus-mob-animation provenance check on top of CLAW's existing priority, blend-weight, and speed validation. Quarantine is limited to that attacker's animation channel; it drains queued defense work and forces a safe block release while stronger part, effect, and projectile evidence remains live. The real Lycoris client-effect channels, overlapping-evidence merge, reserved trusted-evidence capacity, live target distance, hitbox-CFrame correction, replicated state refresh, and character-removal cleanup remain in place. Debug and copied diagnostics now report raw bursts, paced churn, rearm drops, early aborts, and safety releases separately.
