...
Every launch starts inert: Burster, every burst rule, animation logging, Ghost Fire, auto-defense, attack/combat assistance, probability, optional filters/fallbacks, diagnostics, and debug blocks are reset to OFF even if an older settings file saved them as enabled. Tuning values, target lists, key bindings, and explicitly selected action-roll targets remain saved. Enabling Auto Defense or an assistance feature automatically enables its required Combat master. The current v0.4.2 build restores Lycoris's state-driven Unblock contract: CLAW keeps releasing at a throttled rate until the replicated `Blocking` effect actually disappears, preventing a drained queue from trapping weapon-slot and attack input. It retains v0.4.1's per-attacker and same-animation rearm leases, paced-admission and early-abort containment, player-versus-mob-animation provenance check, input-safe quarantine, trusted-evidence bypass, and expanded diagnostics.
